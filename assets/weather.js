...
function getCity(coordinates) {
  ...
  xhr.open(
    "GET",
    "https://us1.locationiq.com/v1/reverse.php?key=pk.ebd508c55452e90db277f28948e222bb&lat=" +
      lat +
      "&lon=" +
      lng +
      "&format=json",
  ...
  $.ajax({
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (results[i].dt_txt.indexOf("15:00:00") !== -1) {
        ...
        const cityDate = $("<h4>")
          .addClass("card-title")
          .text(new Date(results[i].dt_txt).toLocaleDateString("en-US"));
        const temperature = $("<p>")
          .addClass("card-text forecastTemp")
          .text("T: " + tempF + " °F");
        const humidity = $("<p>")
          .addClass("card-text forecastHumidity")
          .text("H: " + results[i].main.humidity + "%");
